...
Module SpecParams <: SpecParams_sig.
  ...
  Definition create_initial_state_fn (spec_turn: Spec.turn_t) (ready_tag: Tag) : reg_t -> nat -> list bool :=
    (fun r sz => if Nat.eqb r Impl.REG_clk then
                (if spec_turn then [true] else [false])
              else if Nat.eqb r Impl.REG_ext_ready_for_job then
                   (match ready_tag with
                    | Tag0 => [true; false]
                    | Tag1 => [true; true]
                    end)
              else if Nat.eqb r Impl.REG_ext_output_reg then (* for convenience in spec invariant; doesn't matter b/c we run for one cycle *)
...
Lemma unsafe_get_valid_relates_output_get_valid:
  forall v,
  unsafe_get_field (struct_fields Impl.S_maybe_sz) FIELD_maybe_valid v = Ob~1 ->
        ...
        { destruct_with_eqn (bits_eq (unsafe_get_job_stage St__Step0.[Impl.REG_job0]) (unsafe_get_job_stage St__Boxes.[Impl.REG_job0])).
          ...
          { destruct_match.
            - apply bits_eq_iff in Heqb0.
              ...
              assert (unsafe_get_field (struct_fields Impl.S_maybe_sz) FIELD_maybe_valid
