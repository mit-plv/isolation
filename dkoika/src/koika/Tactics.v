...
Ltac compute_change t1 t2 :=
 ...
 Ltac split_ands_in_hyp H :=
  ...
  Ltac safe_unfold def H :=
    revert H; unfold def; intros H.
  ...
  Ltac simplify_bool :=
    repeat match goal with
           ...
           | H: _ || false = _ |- _ => rewrite orb_false_r in H
  ...
  Ltac simplify_eq_dec:=
    repeat match goal with
           | H: beq_dec _ _ = true |- _ => rewrite beq_dec_iff in H
           | H: beq_dec _ _ = false |- _ => rewrite beq_dec_false_iff in H
           end.
  ...
  Ltac simplify :=
    ...
    | _ => progress simplify_N
    | _ => progress simplify_bool
    | _ => progress simplify_eq_dec
