...
Section Lists.
Lemma Lift_Forall:
  forall {X} f g (ps: list X),
  (forall p, f p <-> f (g p)) ->
  Forall (fun p => f p) ps <->
  Forall (fun p => f p) (map g ps).
Proof.
  intros. repeat rewrite Forall_forall.  split; propositional.
  - rewrite in_map_iff in *. propositional. rewrite<-H. auto.
  - specialize H0 with (x := g x). rewrite<-H in H0. apply H0.
...
Qed.
  ...
  Lemma list_sum_nth_error_ge:
    forall i dstruct_fields len,
      nth_error dstruct_fields i = Some len ->
      list_sum dstruct_fields >= list_sum (firstn i dstruct_fields) + len.
  Proof.
    induction i; unfold list_sum.
    - intros. simpl. destruct dstruct_fields; simpl in *; auto.
      + discriminate.
      + option_simpl; subst. simpl. apply fold_left_add_ge_base.
    - simpl. intros. destruct dstruct_fields; [discriminate | ]. simpl.
  ...
  Lemma ltb_antisym :
    forall T (ltb: T -> T -> bool) (eq_dec: EqDec T) (order: strict_order ltb) k1 k2,
    beq_dec k1 k2 = false ->
    ltb k1 k2 = negb (ltb k2 k1).
  Proof.
    intros T ltb eq_dec order k1 k2 Hneq.
    rewrite beq_dec_false_iff in Hneq.
    destruct_with_eqn (ltb k2 k1); destruct_with_eqn (ltb k1 k2); simpl in *; auto.
    - specialize ltb_trans with (1 := Heqb) (2 := Heqb0). intros. rewrite (ltb_antirefl _ )in H. discriminate.
    - specialize ltb_total with (1 := Heqb) (2 := Heqb0). intros; subst. congruence.
  Qed.

  Section Ordered.
    Context {T: Type}.
    Context {ltb: T -> T -> bool}.
    Context (order: strict_order ltb).
    Fixpoint sorted (m: list T) : bool :=
      match m with
      | x::((y::zs) as m') => andb (ltb x y) (sorted m')
      | _ => true
      end.
  ...
  Lemma N_ltb_trans: forall k1 k2 k3 : N, (k1 <? k2)%N = true -> (k2 <? k3)%N = true -> (k1 <? k3)%N = true.
  Proof.
    intros. apply N.ltb_lt in H. apply N.ltb_lt in H0. apply N.ltb_lt. lia.
  Qed.

  Lemma N_ltb_total: forall k1 k2 : N, (k1 <? k2)%N = false -> (k2 <? k1)%N = false -> k1 = k2.
  Proof.
    intros. apply N.ltb_nlt in H. apply N.ltb_nlt in H0. lia.
  Qed.

  Instance N_strict_order: strict_order N.ltb.
  Proof.
    constructor.
    - exact N.ltb_irrefl.
    - apply N_ltb_trans.
  ...
  Definition map_fst {X Y Z} (f: X -> Z) (xs: list (X * Y)) : list (Z * Y) :=
    map (fun '(x,y) => (f x, y)) xs.

  Lemma Forall_map_snd:
    forall {X Y} (ps: list (X * Y)) f,
    Forall (fun '(_, p) => f p) ps <->
    Forall (fun p => f p) (map snd ps) .
  Proof.
    intros. repeat rewrite Forall_forall.
    split; intros.
    - rewrite in_map_iff in H0. propositional. specialize H with (1 := H2). destruct_match_pairs; simplify. auto.
    - destruct_match_pairs; subst. eapply H. rewrite in_map_iff. eexists; split; eauto; reflexivity.
  Qed.

  Lemma Forall_ignore_map_fst:
    forall {X Y Z} f (g: X -> Z) (ps: list (X * Y)),
    Forall (fun '(_, p) => f p) (map_fst g ps) <->
    Forall (fun '(_, p) => f p) (ps).
  Proof.
    intros. unfold map_fst. repeat rewrite Forall_forall.
    split; propositional; repeat (destruct_match_pairs; propositional; try rewrite in_map_iff in *); simplify.
    - specialize (H (g x0, y)). simpl in H. apply H. rewrite in_map_iff. eexists; split; simpl; eauto; reflexivity.
    - specialize (H _ H2). simpl in *. auto.
  Qed.
Lemma unit_equiv:
  forall {X} (x y: X),
  [x] = [y] <-> x = y.
Proof.
  split; propositional; try rewrite_solve.
  inversions H. reflexivity.
Qed.
Lemma In_disjoint:
  forall {A: Type} (EqDec: EqDec A) (x: A) xs1 xs2,
  forallb (fun x => negb (is_someb (find (beq_dec x) xs2))) xs1 = true ->
  In x xs1 ->
  In x xs2 ->
  False.
Proof.
  intros. rewrite forallb_forall in *. specialize H with (1 := H0).
  rewrite negb_true_iff in *. consider @is_someb. bash_destruct H.
  eapply find_none in Heqo; eauto. simplify. contradiction.
Qed.
Lemma In_subset :
  forall {A: Type} (EqDec: EqDec A) (x: A) xs1 xs2,
  forallb (fun x => (is_someb (find (beq_dec x) xs2))) xs1 = true ->
  In x xs1 ->
  In x xs2.
Proof.
  intros. rewrite forallb_forall in *. specialize H with (1 := H0).
  consider @is_someb. bash_destruct H.
  eapply find_some in Heqo; eauto. propositional; simplify. auto.
Qed.
Lemma In_remove_reg:
  forall {T: Type} {EqDec: EqDec T} (reg: T) reg' (reg_list: list T),
  In reg reg_list ->
  reg <> reg' ->
  In reg (filter (fun r => negb (@beq_dec T _ r reg')) reg_list ).
Proof.
  intros. rewrite filter_In. split; auto. rewrite negb_true_iff.
  rewrite beq_dec_false_iff. auto.
Qed.
  Definition eta_expand_list {T: Type} (v: list T) (len: nat) (default: T) : list T :=
    map (fun n => nth n v default) (seq 0 len).

  Lemma eta_expand_list_correct:
    forall {T} default (v: list T),
    v = eta_expand_list v (Datatypes.length v) default.
  Proof.
    unfold eta_expand_list. induction v using rev_ind; auto.
    rewrite List.app_length. simpl.
    rewrite seq_app. simpl. rewrite map_app.
    f_equal.
    + rewrite IHv at 1. apply map_ext_in.
      intros. rewrite in_seq in H.
      rewrite app_nth1 by lia. auto.
    + simpl. rewrite nth_middle. auto.
  Qed.
...
Section Numbering.
  Context {T: Type}.
  Context {EqDec_T: EqDec T}.

  Definition T_id_nat (tlist: list T) (r: T) : nat :=
    match find_with_index (beq_dec r) tlist with
    | Some (i, _) => i
    | None => 0
    end.

  Definition T_id (tlist: list T) (r: T) : N:=
    N.of_nat (T_id_nat tlist r).
