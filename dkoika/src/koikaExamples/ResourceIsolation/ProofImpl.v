...
Section Util.
  Definition unsafe_get_job_stage vs :=
    unsafe_get_field Impl.S_job_t.(dstruct_fields) Impl.FLD_job_t_stage vs.
  ...
  Lemma regs_in_taint_existsb:
    forall tag reg r1s r2s,
      ...
      existsb (beq_dec reg) (r1s ++ r2s) = true.
  Proof.
  ...
  Qed.
    ...
    Proof.
      intros. unfold unsafe_get_reg, state_set, r_get_reg.
    ...
    Proof.
      ...
      destruct_match; simplify; auto; try discriminate.
    Qed.
         ...
         Post_custom :=
           [(Some DoInput__Job0__Stage,
             fun st st' sigma =>
               ...
               (st.[Impl.REG_ext_ready_for_job] = option_to_maybe' 1 (Some [false]) /\
                (unsafe_get_field (dstruct_fields Impl.S_maybe_job_req) FIELD_maybe_valid (success_or_default (sigma Impl.EXTFN_GetJob Ob~0) Ob)) = Ob~1) ->
           ...
           (Some DoInput__Job1__Stage,
             fun st st' sigma =>
               ...
               (unsafe_get_field (dstruct_fields Impl.S_maybe_job_req) FIELD_maybe_valid (success_or_default (sigma Impl.EXTFN_GetJob Ob~1) Ob) = Ob~1)) ->
            ...
            [(Impl.REG_job0,
             fun st st' sigma v =>
               v = if bits_eq (st.[Impl.REG_ext_ready_for_job]) (option_to_maybe' 1 (Some [false]))
                      && bits_eq (unsafe_get_field (dstruct_fields Impl.S_maybe_job_req) FIELD_maybe_valid (success_or_default (sigma Impl.EXTFN_GetJob Ob~0) Ob)) Ob~1 then
                     v
           ...
           ;(Impl.REG_job1,
             fun st st' sigma v =>
               v = if bits_eq (st.[Impl.REG_ext_ready_for_job]) (option_to_maybe' 1 (Some [true]))
                      && bits_eq (unsafe_get_field (dstruct_fields Impl.S_maybe_job_req) FIELD_maybe_valid (success_or_default (sigma Impl.EXTFN_GetJob Ob~1) Ob)) Ob~1 then
                     v
         ...
         Post_custom := [(Some DoStep0__Job0__Stage,
                        ...
                        ;(Some DoStep0__ext_output_reg,
                           fun st st' sigma =>
                             ...
                             unsafe_get_field (dstruct_fields Impl.S_maybe_sz) FIELD_maybe_valid st'.[Impl.REG_ext_output_reg0] = Ob~1)
         ...
         Post_custom := [(Some DoStep1__Job1__Stage,
                        ...
                        ;(Some DoStep1__ext_output_reg,
                           fun st st' sigma =>
                             ...
                             unsafe_get_field (dstruct_fields Impl.S_maybe_sz) FIELD_maybe_valid st'.[Impl.REG_ext_output_reg1] = Ob~1)
...
Module ScheduleProofs (ImplProofs: ImplementationProofs_Sig).
  ...
  Lemma Invariant_step_preserved'':
    forall impl_st (sigma: input_t -> ext_sigma_t) input log,
      ...
      interp_scheduler (sigma input) Impl.int_fn_env Impl.struct_env impl_st Impl.rules Impl.schedule = Success log ->
      WF_log Impl.reg_types log.(Log__koika) ->
      WF_state Impl.reg_types (commit_update impl_st log.(Log__koika)) ->
      ImplInvariant (commit_update impl_st log.(Log__koika)) /\
      ImplPostCond impl_st (commit_update impl_st log.(Log__koika)) (sigma input).
  Proof.
    ...
    assert (oraat_interp_scheduler' (sigma input) Impl.int_fn_env Impl.struct_env Impl.rules
            (commit_update impl_st log_empty) true Impl.schedule = (true, commit_update impl_st log.(Log__koika)))
      as Horaat.
    ...
    - intros tag. unfold ImplInvariant__NONE_OR_INIT.
      ...
      + rewrite_no_write Impl.REG_job0 Post__UpdateReady.
        ...
        split_ors_in Htaint_reg; try discriminate; simplify; subst.
...
Module ImplementationProofs : ImplementationProofs_Sig.
    ...
    Arguments dstruct_fields : simpl never.
            ...
            { repeat (repeat step_wp_oraat_safe; simpl).
              ...
              { split; [trivial | ].
                ...
                + intros Hstage1.
                  destruct_match; auto.
                  exfalso.
                  rewrite case_G with (bs := (unsafe_get_field (dstruct_fields Impl.S_job_t) Impl.FLD_job_t_stage st.[Impl.REG_job0])) in Heqb; auto; try solve_bits; [discriminate | ].
                  apply unsafe_get_field_length; auto.
                ...
                + simpl. rewrite andb_true_r.
                  rewrite case_G with (bs := (unsafe_get_field (dstruct_fields Impl.S_job_t) Impl.FLD_job_t_stage st.[Impl.REG_job0])); auto; try solve_bits; [ | apply unsafe_get_field_length ;auto]. simpl.
            ...
            { repeat (repeat step_wp_oraat_safe; simpl).
              ...
              { split; [trivial | ].
                ...
                + intros Hstage1.
                  destruct_match; auto.
                  exfalso.
                  rewrite case_G with (bs := (unsafe_get_field (dstruct_fields Impl.S_job_t) Impl.FLD_job_t_stage st.[Impl.REG_job1])) in Heqb; auto; try solve_bits; [discriminate | ].
                  apply unsafe_get_field_length; auto.
                + intros. simplify_updates.
                + intros.
                ...
                + intros Hstage. destruct Hstage as [? | Hstage_eq]; [ congruence | ].
                  ...
                  * move Match_clk_and_G1Box_can_receive at bottom.
                    ...
                    { discriminate. }
                + simpl. rewrite andb_true_r.
                  rewrite case_G with (bs := (unsafe_get_field (dstruct_fields Impl.S_job_t) Impl.FLD_job_t_stage st.[Impl.REG_job1])); auto; try solve_bits; [ | apply unsafe_get_field_length ;auto]. simpl.
...
Section TopLevelProofs.
  ...
  Proof.
    ...
    - unfold Impl.initial_state; intros; destruct_match; simplify; auto.
  ...
  Proof.
    ...
    assert (WF_log Impl.reg_types log.(Log__koika)) by (eapply Proofs.Step_WF_Log; eauto).
