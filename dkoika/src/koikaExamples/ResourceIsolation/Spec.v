...
Module Spec.
  ...
  Section WithParams.
    ...
    (* TODO: a bit awkward/not fully generic. Allowed to decide next job based on whether busy or not. *)
    ...
    Record state_t : Type :=
      { state0: local_machine_state_t;
        ...
        hist_ready_for_job: option Tag; (* at start of prev cycle *)
        turn: turn_t
      }.
    ...
    (* Output: do transition at start of next cycle -- slightly weird *)
    ...
    (* Compute (option_to_maybe (STRUCT_maybe 0 1).(dstruct_fields) (Some [false])). *)
