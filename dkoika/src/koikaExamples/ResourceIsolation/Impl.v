...
Module Impl.
  ...
  Lemma N_ltb_succ:
    forall n, (n <? N.succ n)%N = true.
  Proof.
    intros. apply N.ltb_lt. lia.
  Qed.
  ...
  Section TypeDefs.
    ...
    Definition S_job_req_id := 0%N.
    Definition FLD_job_req_n := 0%N.
    Definition FLD_job_req_m := 1%N.
    Definition FLD_job_req_x := 2%N.

    Definition S_job_req :=
      {| dstruct_name := _Struct "job_req" S_job_req_id;
         dstruct_fields := [(_StructField "n" FLD_job_req_n, ctr_sz)
                          ;(_StructField "m" FLD_job_req_m, ctr_sz)
                          ;(_StructField "x" FLD_job_req_x, sz)
                          ]
      |}.

    Definition S_maybe_job_req_id := 1%N.
    Definition S_maybe_job_req :=
      STRUCT_maybe (_Struct "maybe_job_req" S_maybe_job_req_id) (struct_sz S_job_req).

    Definition S_maybe_1_id := 2%N.
    Definition S_maybe_1 :=
      STRUCT_maybe (_Struct "maybe_1" S_maybe_1_id) 1.

    Definition S_maybe_sz_id := 3%N.
    Definition S_maybe_sz :=
      STRUCT_maybe (_Struct "maybe_sz" S_maybe_sz_id) sz.

    Definition S_job_t_id := 4%N.
    Definition FLD_job_t_stage := 0%N.
    Definition FLD_job_t_job_info := 1%N.

    Definition S_job_t :=
      {| dstruct_name := _Struct "job_t" S_job_t_id;
         dstruct_fields := [(_StructField "stage" FLD_job_t_stage, ENUM_stage_sz)
                          ;(_StructField "job_info" FLD_job_t_job_info, struct_sz S_job_req)
                          ]
      |}.

    Definition S_box_req_id := 5%N.
    Definition FLD_box_req_num_iters := 0%N.
    Definition FLD_box_req_x := 1%N.
    Definition S_box_req_t :=
      {| dstruct_name := _Struct "box_req" S_box_req_id;
         dstruct_fields := [(_StructField "num_iters" FLD_box_req_num_iters, ctr_sz)
    ...
    (* Checks *)
    Goal (sorted (map dstruct_name struct_env) = true). Proof. reflexivity. Qed.
    Goal (forallb (fun s => sorted (map fst s.(dstruct_fields))) struct_env = true). Proof. reflexivity. Qed.

  End TypeDefs.

  Section ExtFnDefs.
    Definition EXTFN_F0 := _ExtFn "F0" 0%N.
    Definition EXTFN_F1 := _ExtFn "F1" 1%N.
    Definition EXTFN_G0 := _ExtFn "G0" 2%N.
    Definition EXTFN_G1 := _ExtFn "G1" 3%N.
    Definition EXTFN_GetJob := _ExtFn "GetJob" 4%N.
  ...
  Module IterBox (sig: IterBox_sig).
    ...
    Definition FN_can_send_req_id := (sig.int_fn_base + 1)%N.
    Definition FN_send_req_id := (sig.int_fn_base + 2)%N.
    Definition FN_can_receive_resp_id := (sig.int_fn_base + 3)%N.
    Definition FN_receive_resp_id := (sig.int_fn_base + 4)%N.
  ...
  Definition F0_reg_base := 0%N.
  Definition F0_fn_base := 0%N.
  ...
  Definition F1_reg_base := Eval compute in (F0_reg_base + (N.of_nat (List.length (F0Box.reg_tys_list))))%N.
  Definition F1_fn_base := Eval compute in (F0_fn_base + (N.of_nat (List.length (F0Box.int_fns))))%N.
  ...
  Definition G0_reg_base := Eval compute in (F1_reg_base + (N.of_nat (List.length (F1Box.reg_tys_list))))%N.
  Definition G0_fn_base := Eval compute in (F1_fn_base + (N.of_nat (List.length (F1Box.int_fns))))%N.
  ...
  Definition G1_reg_base := Eval compute in (G0_reg_base + N.of_nat (List.length (G0Box.reg_tys_list)))%N.
  Definition G1_fn_base := Eval compute in (G0_fn_base + N.of_nat (List.length (G0Box.int_fns)))%N.
  ...
  Definition _reg_base := Eval compute in (G1_reg_base + N.of_nat (List.length (G1Box.reg_tys_list)))%N.
  Definition _fn_base := Eval compute in (G1_fn_base + N.of_nat (List.length (G1Box.int_fns)))%N.
  ...
  Section Rules.
    ...
    Definition update_ready : action :=
      {{ let "job0_st" := read1(REG_job0') in
         let "job1_st" := read1(REG_job1') in
         let "rdy" := `invalid S_maybe_1.(dstruct_name)` in
         if (get_field(S_job_t.(dstruct_name), var "job0_st", FLD_job_t_stage)
               == const ENUM_stage__None) then
           set "rdy" := `valid S_maybe_1.(dstruct_name) {{ Ob~0}}`
         else if (get_field(S_job_t.(dstruct_name), var "job1_st", FLD_job_t_stage)
               == const ENUM_stage__None) then
           set "rdy" := `valid S_maybe_1.(dstruct_name) {{ Ob~1}}`
         else
           pass;
  ...
  Section StateMachine.
    ...
    Definition initial_state : state_t :=
      SortedRegEnv.map (fun r sz => if N.eqb r REG_ext_ready_for_job
  ...
  (* Technically for functional correctness, also want F0=F1 and G0=G1 *)
    Record wf_sigma (sigma: input_t -> ext_sigma_t) : Prop :=
      { wf: forall input, WF_ext_sigma ext_fn_types (sigma input);
        ...
        wf_Constants: forall input1 input2 fn, if N.eqb fn EXTFN_GetJob
                                          then True
...
Module Tests.
  ...
  Definition sigma__fns (input: input_t) : list (ext_fn_t * (vect_t -> result vect_t unit)) :=
    ...
    ;(EXTFN_GetJob, (fun _ => option_to_maybe (S_maybe_job_req.(dstruct_fields)) input))
  ...
  Definition sigma (input: input_t) : ext_sigma_t :=
    fun ext_fn =>
      match find (fun '(fn, _) => N.eqb ext_fn fn) (sigma__fns input) with
  ...
  (* Definition foo := *)
  (*   map (fun value => *)
  (*          let n := to_nat (unsafe_get_field (S_job_req.(dstruct_fields)) FLD_job_req_n value) in *)
  (*          let m := to_nat (unsafe_get_field (S_job_req.(dstruct_fields)) FLD_job_req_m value) in *)
  (*          let x := to_nat (unsafe_get_field (S_job_req.(dstruct_fields)) FLD_job_req_x value) in *)
  (*          (n,m,x) *)
  (*       ) initial_jobs. *)
  ...
  Definition compute_n (n: nat) :=
    match ResourceIsolation.Util.step_n' (koika_step sigma) input_machine n initial_state
