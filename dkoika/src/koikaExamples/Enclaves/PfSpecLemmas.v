...
Lemma post_implies_post_obs_exit:
  ...
  spec_post_obs_exit core machine_st
    ...
    |}
    {|
      obs_mainmem := SemanticUtils.unsafe_get_ext_call_from_log _ext_fn_tys ext_log (_ext ext_mainmem);
      obs_exit_enclave :=
...
Proof.
  ...
  destruct core; unfold local_step_fn0, local_step_fn1, Core0Machine.step, Core1Machine.step in *;
    ...
    specialize mem_push_request__success with (log := (Log__ext s)) (mem := machine_mem_st machine_st); intros hpush;
...
Proof.
  ...
  - constructor; unfold machine_koika_st.
    ...
    + simpl. constructor.
      ...
      * simpl. propositional. discriminate.
      * simpl. propositional. discriminate.
      * unfold WF_dram. simpl. propositional.
        consider get_enclave_dram.
        inversions hmem. consider WF_dram.
        bash_destruct H; eauto.
  - constructor; unfold machine_koika_st.
    + unfold strong_WF_state.  split.
      * reflexivity.
