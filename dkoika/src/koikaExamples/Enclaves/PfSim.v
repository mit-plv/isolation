...
Require Import koikaExamples.Enclaves.PfSimHelpers.
...
Module Pf (EnclaveParams: EnclaveParams_sig)
  ...
  Proof.
    ...
    setoid_rewrite impl_mem_update0. eauto.
  Qed.
  ...
  Record ext_mem_sim (core_id: ind_core_id) (config: enclave_config)
      ...
      { ext_mem_sim__dram: forall addr, addr_in_config EnclaveParams.enclave_sig addr config ->
                                 impl_mem.(ExternalMemory.ext_mem) addr = spec_mem.(ExternalMemory.ext_mem) addr;
        ext_mem_sim__ext: is_mem_core_read_turn' core_id cycles = true ->
                        impl_mem.(ExternalMemory.ext_resp) = spec_mem.(ExternalMemory.ext_resp)
      }.
  ...
  ; Sim_machine_state: forall core_id, machine_sim core_id impl_st
                                          ...
                                          spec_st.(cycles)
  ; Sim_memory_regions: mem_map_equivalent impl_st.(machine_mem_st).(ExternalMemory.ext_mem)
...
Arguments filter_input : simpl never.
  ...
  Proof.
    ...
    - constructor; intros * hconfig; simpl.
...
Definition step_memory_update_sim
  ...
  | CoreState_Enclave koika_st config =>
    ...
    ExternalMemory.ext_mem impl_mem' addr = ExternalMemory.ext_mem (machine_mem_st m) addr
...
Lemma spec_sim_exit:
  ...
  SimPost core impl_st impl_st' machine_st machine_st' config impl_out spec_out cycles ->
...
Ltac simplify_hyps :=
      match goal with
      | H: ?x = Success _, H1: ?x = Success _ |- _ =>
...
Lemma ext_mem_sim_preserved:
  ...
  (is_mem_core_write_turn' core cycles = true ->
    unsafe_get_ext_call_from_log impl_log ((_ext ext_mainmem)) =
    unsafe_get_ext_call_from_log spec_log ((_ext ext_mainmem))) ->
  (_unsafe_get_field mem_input "put_valid"
                        (unsafe_get_ext_call_from_log impl_log (_ext ext_mainmem)) = Ob~0 \/
    is_mem_core_write_turn' core cycles = true \/
    is_mem_core_write_turn' (other_core core) cycles = true)  ->
  (_unsafe_get_field mem_input "put_valid"
                                (unsafe_get_ext_call_from_log spec_log (_ext ext_mainmem)) = Ob~0 \/
    is_mem_core_write_turn' core cycles = true) ->
  (is_mem_core_write_turn' (other_core core) cycles = true ->
   match get_valid_addr_from_push_req
     (unsafe_get_ext_call_from_log impl_log ((_ext ext_mainmem))) with
  ...
  ExternalMemory.mainmem__ext
                (unsafe_get_ext_call_from_log impl_log (_ext ext_mainmem))
                impl_mem = Success impl_mem' ->
  ExternalMemory.mainmem__ext
                (unsafe_get_ext_call_from_log spec_log (_ext ext_mainmem))
                spec_mem = Success spec_mem' ->
...
Proof.
  ...
  destruct (beq_dec (_unsafe_get_field mem_input "put_valid"
                        (unsafe_get_ext_call_from_log impl_log (_ext ext_mainmem))) Ob~0 ) eqn:himpl_valid;
  destruct (beq_dec (_unsafe_get_field mem_input "put_valid"
                        (unsafe_get_ext_call_from_log spec_log (_ext ext_mainmem))) Ob~0 ) eqn:hspec_valid;
            simplify; propositional;
    consider @_unsafe_get_field; consider @success_or_default; bash_destruct himpl_valid; bash_destruct hspec_valid; simplify.
  - constructor; simpl; auto.
    eapply ext_mem_sim__dram; eauto.
  - split_ors_in hspec_zeroes; [subst; contradiction | ]. propositional.
  ...
  - bash_destruct hspec. bash_destruct himpl. simplify.
    ...
    repeat rewrite hpush_sim in *. rewrite Heqr3 in *.
    constructor; auto. simpl. intros.
    + consider ExternalMemory.update_dram. simplify. destruct_matches_in_hyp himpl; simplify; simpl; auto.
      * eapply ext_mem_sim__dram; eauto.
      * destruct_match; auto.
        eapply ext_mem_sim__dram; eauto.
    + consider ExternalMemory.update_dram. simplify. destruct_matches_in_hyp himpl; simplify; auto.
      simpl. intros.
      unfold get_valid_addr_from_push_req in hcore_addr.
      simpl_match. simpl_match. rewrite Heqr0 in hcore_addr.
      erewrite ext_mem_sim__dram; eauto.
Qed.
Import SymbSimDefs.
  ...
  - repeat rewrite Forall_app in H. destruct H as (Hinv & Hsm & Hmem).
    ...
    + clear Hinv Hmem. revert Hsm.
      ...
      all: rewrite hdata_enc in Hconfig; consider _smid;
           ...
           intros _ * hlen' hconfig; setoid_rewrite hdata_enc in hconfig;
           set (spec_st.[_]) as enc_data in *;
           clearbody enc_data;
...
(* Lemma of_nat_to_nat__2: *)
(*   of_nat 2 (Nat.modulo cycles0 4) = mem_core0_write_turn *)
...
Lemma SimPre_implies_SimPreInvariants:
  forall core impl_koika spec_koika impl_sigma spec_sigma,
  SimPre EnclaveParams.enclave_sig core impl_koika spec_koika impl_sigma spec_sigma ->
  SimPreInvariants  core impl_koika spec_koika impl_sigma spec_sigma.
Proof.
  consider SimPre. consider SimPreInvariants.
  intros * hpre. consider sim_pre_conds. apply Forall_app  in hpre; propositional.
Qed.
...
Lemma SimPre_stay_implied:
  ...
  ExternalMemory.mainmem__ext (unsafe_get_ext_call_from_log (Log__ext spec_log) (_ext ext_mainmem))
                 (machine_mem_st machine_st) = Success spec_ext_mem' ->
  ExternalMemory.mainmem__ext (unsafe_get_ext_call_from_log (Log__ext log) (_ext ext_mainmem))
  ...
  (forall addr : N, addr_in_config addr config ->
               ExternalMemory.ext_mem (machine_mem_st impl_st) addr = ExternalMemory.ext_mem (machine_mem_st machine_st) addr) ->
  SimPre EnclaveParams.enclave_sig core (commit_update (machine_koika_st impl_st) (Log__koika log))
    ...
    (mk_sigma_fn spec_ext_mem'
       (filter_input
          ...
          input)).
Proof.
  intros * hwf_impl hwf_impl' HImplPost HSimPre HSimPost hobserve hspec_push himpl_push hconfig hdram.
  prop_pose_with_custom hsm_sim' (CustomSmSim SmSim__Regs) HSimPost. cbn - [_id sm_sim_regs] in hsm_sim'.
  prop_pose_with_custom hsm_sim (CustomSmSim SmSim__Regs) HSimPre . cbn - [_id sm_sim_regs] in hsm_sim.
  prop_pose_with_custom hmem_sim (CustomMemSim MemSim__Regs) HSimPre. cbn - [_id mem_sim_regs] in hmem_sim.
  prop_pose_with_custom hmem_sim' (CustomMemSim MemSim__Regs) HSimPost. cbn - [_id mem_sim_regs] in hmem_sim'.
  ...
  + unfold sim_mem_pre_conds.
    assert ((commit_update (machine_koika_st impl_st) (Log__koika log)).[_rid (Memory Memory.turn)] = mem_core_read_turn core ->
             (unsafe_get_ext_call_from_log (Log__ext log) (_ext ext_mainmem)) =
             (unsafe_get_ext_call_from_log (Log__ext spec_log) (_ext ext_mainmem))) as hpush_sim.
    ...
    * intros * hlen H. simpl. cbn - [_id _sid] in H.
      propositional. rewrite<-hpush_sim in *.
    ...
    * intros * hlen; intros; propositional.
...
Ltac solve_In_to_find' :=
  apply In_exists; vm_reflect.
        ...
        2: { destruct core; solve_In_to_find'. }
        destruct core; cbn; intros; simplify;
          setoid_rewrite H2 in H; contradiction.
      }
    * intros ? hlen hvalid hturn.
      prop_apply_with_custom (CustomMemSim (MemImplExtCorrectCore CoreId0)) HSimPost; auto.
      cbn - [_id].
      eapply mem_get_response_valid; eauto.
    * intros ? hlen hvalid hturn.
      prop_apply_with_custom (CustomMemSim (MemImplExtCorrectCore CoreId1)) HSimPost; auto.
      cbn - [_id].
      eapply mem_get_response_valid; eauto.
Qed.
...
Proof.
  ...
  - intros. eapply SimPre_stay_implied; auto.
    ...
    * eapply impl_mem_update; eauto.
    * eapply running_sim__ghost; eauto.
    * eapply ext_mem_sim__dram. eapply running_sim__ext_mem; eauto.
  - destruct core.
  ...
  - eapply ext_mem_sim_preserved with (1 := running_sim__ext_mem _ _ _ _ _ Hrunning_sim)
    ...
    + apply impl_mem_update with (1 := himpl_post).
    + auto.
  - reflexivity.
  - reflexivity.
Unshelve.
...
Proof.
  ...
  - specialize spec0_interp_cycle_correct with
    ...
    simpl in hspec_step. consider ExternalMemory.update_memory.
    specialize mem_push_request__success with (mem := machine_mem_st machine_st) (log := Log__ext s); intros hupdate.
    repeat (assert_pre_and_specialize hupdate; [solve[eauto with modularity] | ]). simplify.
    setoid_rewrite Heqr1 in hspec_step. simplify.
...
Lemma step_exit_sim :
  ...
  impl_is_waiting core
   ...
   (forall addr : N, addr_in_config addr config -> ExternalMemory.ext_mem mem_st' addr = ExternalMemory.ext_mem (machine_mem_st machine_st') addr).
Proof.
...
Lemma ext_mem_unchanged_at_addr:
  ...
  mem_push_in_config (machine_koika_st impl_st)
            (unsafe_get_ext_call_from_log (log) (_ext ext_mainmem)) ->
  ExternalMemory.ext_mem mem_st' addr = ExternalMemory.ext_mem (machine_mem_st impl_st) addr.
Proof.
...
Lemma enter_enclave_sim:
  ...
  (forall addr, addr_in_config addr new ->
           (machine_mem_st impl_st).(ExternalMemory.ext_mem) addr =
             (get_enclave_dram EnclaveParams.enclave_sig spec_mem new) addr) ->
  ImplPost impl_st
            {|
              machine_koika_st := commit_update (machine_koika_st impl_st) (Log__koika log);
              machine_mem_st := mem_st'
...
Proof.
  ...
  pose proof (EnclaveParams.pf_disjoint_configs) as pf_disjoint. consider wf_disjoint_configs.
...
Lemma enter_mem_sim :
  ...
  mem_map_equivalent (ExternalMemory.ext_mem (machine_mem_st impl_st)) (get_spec_configs spec_st) memory0 ->
  can_enter_enclave new (get_impl_config impl_st.(machine_koika_st) (other_core core) SecurityParams.extract_rf) = true ->
  ...
  addr_in_config addr new ->
  ExternalMemory.ext_mem (machine_mem_st impl_st) addr = get_enclave_dram EnclaveParams.enclave_sig memory0 new addr.
Proof.
...
Lemma sim_local_step:
  ...
  mem_map_equivalent impl_st.(machine_mem_st).(ExternalMemory.ext_mem)
                     (get_spec_configs spec_st)
                     memory0 ->
  ...
  step_memory_update_sim core (machine_state spec_st core) (machine_mem_st impl_st')
...
Proof.
  ...
  specialize PfImplLemmas.impl_step_preserves_impl_inv' with (1 := Sim_impl_invariant _ _ Hsim) (input := i_get_output input_machine ext_st). propositional. setoid_rewrite H0 in Heqr0. simplify. rename H0 into Himpl_step. rename H2 into Hpost. simpl.
  ...
  unfold get_observations in *. simpl in Heqr1. consider ExternalMemory.update_memory.
  setoid_rewrite Heqr1 in Hupdate. simplify.
...
Lemma mem_map_equivalent_update_config:
  ...
  mem_map_equivalent (ExternalMemory.ext_mem (machine_mem_st impl_st)) (get_spec_configs spec_st)
...
Lemma intermediate_mem_map_equivalent:
  ...
  case_core_state core
            ...
            memory0 memory1 (get_spec_configs spec_st (other_core core)) ->
  mem_map_equivalent (ExternalMemory.ext_mem (machine_mem_st impl_st)) (get_spec_configs spec_st) memory1.
Proof.
...
Qed.
     ...
     Lemma impl_step_memory_obs:
       ...
       valid_local_obs_push_request impl_st (obs_mainmem local_obs) true true = true.
     Proof.
...
Lemma sim_addr_not_in_config:
  ...
  ExternalMemory.ext_mem (machine_mem_st impl_st') addr = ExternalMemory.ext_mem (machine_mem_st impl_st) addr.
Proof.
  consider @Impl.step.
  ...
  - consider get_spec_configs.
    consider ExternalMemory.update_dram. simplify.
    bash_destruct Heqr0; simplify; simpl; auto.
    pose proof (Sim_machine_state _ _ hsim CoreId0) as machine_sim0.
    pose proof (Sim_machine_state _ _ hsim CoreId1) as machine_sim1.
    intros hwf_mem.
    (* destruct_match; simplify; auto. *)
    ...
    + setoid_rewrite Heqr4 in hwf_mem1.
...
Qed.
  ...
  Proof.
    intros * hdram. unfold Impl.step_n, Spec.step_n, step_n, safe_step_n, Machine.step_n.
    unfold Machine.step_n. destruct_match_pairs; simplify.
    specialize step_n'_sim with (1 := @InitialSim initial_dram hdram) (2 := Heqp).
