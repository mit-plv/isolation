...
counterexample_lines=$(cloc "CounterexampleDemo.v" --csv --quiet | tail -n 1 | cut -d ',' -f 5)

#monolithic=$(($all_lines - $spec_lines-$modular_lines-$counterexample_lines))
#modular=$(($all_lines - $spec_lines-$monolithic_lines-$counterexample_lines))
total=$(($all_lines - $spec_lines-$counterexample_lines))

echo "Proof lines: total:$total" 
