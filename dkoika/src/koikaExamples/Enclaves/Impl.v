(*! Conversion to dkoika + model of external memory *)
...
Module KEnclaveParams (EnclaveParams: EnclaveParams_sig) <: rv_isolation.Common.EnclaveParams_sig.
  ...
  Definition core_init0 : rv_isolation.Common.core_init_params_t :=
    {| rv_isolation.Common.machine_pc :=
        eq_rect _ _ (vect_of_list (enclave_sig.(_core_init0).(machine_pc)))  _ (wf_sig.(wf_core_init0 _));
      rv_isolation.Common.machine_config := enclave_sig.(_core_init0).(machine_config)
    |}.
  Definition core_init1 : rv_isolation.Common.core_init_params_t :=
    {| rv_isolation.Common.machine_pc :=
        eq_rect _ _ (vect_of_list (enclave_sig.(_core_init1).(machine_pc)))  _ (wf_sig.(wf_core_init1 _));
      rv_isolation.Common.machine_config := enclave_sig.(_core_init1).(machine_config)
    |}.

  Definition enclave_params: rv_isolation.Common.enclave_params_sig :=
  {| rv_isolation.Common._enclave_base := enclave_base;
     rv_isolation.Common._enclave_size := enclave_size;
     rv_isolation.Common._enclave_bootloader_addr := enclave_bootloader_addr;
     rv_isolation.Common._shared_region_base := shared_region_base;
     rv_isolation.Common._shared_region_size := shared_region_size;
     rv_isolation.Common._core_init0 := core_init0;
     rv_isolation.Common._core_init1 := core_init1
  |}.
End KEnclaveParams.

Module DummyEnclaveParams <: rv_isolation.Common.EnclaveParams_sig.
  Definition enclave_params :=
    {| rv_isolation.Common._enclave_base := fun _ => @Bits.zero 32;
       rv_isolation.Common._enclave_size := fun _ => @Bits.zero 32;
       rv_isolation.Common._enclave_bootloader_addr := fun _ => @Bits.zero 32;
       rv_isolation.Common._shared_region_base := fun _ => @Bits.zero 32;
       rv_isolation.Common._shared_region_size := @Bits.zero 32;
       rv_isolation.Common._core_init0 :=
        {| rv_isolation.Common.machine_pc := @Bits.zero 32;
          rv_isolation.Common.machine_config := None
        |};
       rv_isolation.Common._core_init1 :=
        {| rv_isolation.Common.machine_pc := @Bits.zero 32;
          rv_isolation.Common.machine_config := None
        |};
    |}.
...
Definition ext_fn_t := rv_isolation.Common.ext_fn_t.
...
Module Type Impl (EnclaveParams: EnclaveParams_sig).
  Module EnclaveParams' <: rv_isolation.Common.EnclaveParams_sig := KEnclaveParams EnclaveParams.
...
Module ExternalMemory.
  ...
  Record external_mem_t :=
    { ext_resp: option (mem_resp_t); (* previous req *)
      (* pf_wf_ext_req: List.length ext_req = struct_sz mem_input; *)
      ext_mem: dram_t
    }.
  Definition initial_mem (dram: dram_t) : external_mem_t :=
    {| ext_resp := None; (* mem_input *)
       ext_mem := dram
    |}.

  Definition mem_get_response__koika (mem: external_mem_t) : result vect_t unit :=
  ...
  Definition update_dram (req:vect_t) (dram: dram_t) : result external_mem_t unit :=
    ...
    if eq_dec (to_nat byte_en) 0 then (* LOAD *)
  ...
  Definition mainmem__ext (obs_mainmem: vect_t) (mem: external_mem_t): result external_mem_t unit :=
  ...
  Definition update_memory (obs_mainmem: vect_t)
                           (mem: external_mem_t)
                           : result external_mem_t unit :=
    mainmem__ext obs_mainmem mem.
  ...
  Definition mk_sigma_fn : ExternalMemory.external_mem_t -> input_t -> ext_sigma_t :=
      fun mem input (fn: @Syntax.ext_fn_t N) v =>
        if beq_dec fn (_ext ext_mainmem) then
          ExternalMemory.mem_get_response__koika mem
...
Module Type TopLevelModule (EnclaveParams: EnclaveParams_sig) (Impl: Impl EnclaveParams).
  ...
  Import Machine.SM. Import Machine.
    Section WithSchedule.
      ...
      Definition machine_step_local_obs (sigma: input_t -> ext_sigma_t) (st: machine_state_t) (input: input_t)
        ...
        let/res mem' := update_memory obs.(obs_mainmem) st.(machine_mem_st) in
  ...
  Section Schedules.
    ...
    Definition mem_schedule core : @scheduler mem_rules_t :=
      Rl_doMemReq core |> Rl_doPurge core |> Rl_doExternalMem
                   |> Rl_doMemResp core |> Rl_doInit core |> Rl_tick |> done.
  ...
  (* Instantiation of spec machine 0 *)
  Module Core0Machine.
    Import Machine.
    Section Typecheck.
  ...
  (* Instantiation of spec machine 1 *)
  Module Core1Machine.
    Import Machine.
    Section Typecheck.
