...
End SymbPfInit.
Lemma forall_interp_spred2_same_pkg:
  forall ps pkg, Forall (interp_spred2 pkg pkg) ps <-> Forall (interp_spred pkg ) ps.
Proof.
...
Module PfInit (EnclaveParams: EnclaveParams_sig)
  ...
  Proof.
    ...
    - constructor; simpl; auto; propositional; try discriminate.
    - repeat constructor; simpl; auto; propositional; simplify; try discriminate; auto.
  Qed.
  ...
  Lemma wf_initial_cache_pair:
    forall (cache : mem_type) (core : ind_core_id),
    wf_cache_pair cache core
      (initial_koika_state (_core_init0 EnclaveParams.enclave_sig) (_core_init1 EnclaveParams.enclave_sig)
         Impl.init_mem_turn Impl.init_sm_turn) ExternalMemory.initial_cache_pair.
  Proof.
    intros. constructor; auto.
    all: unfold cache_flush_line, meta_flush_line; simpl; propositional; try discriminate.
    simplify; auto.
    basic_cbn. unfold MemoryProofs.MemSymbDefs.meta_resp_in_range_with_line.
    unfold dummy_interp_spred_at_st. intros.
    basic_cbn. intro. vm_compute in H. discriminate.
  Qed.
  ...
  Proof.
    ...
    { eapply ImplInvariant_spreds_implied with (input := dummy_input_t)
      ...
      - unfold dummy_interp. intros.
        rewrite<-forall_preprocess_fancy_spreds_correct.
        pose proof cache_pre_conds_implied.
        rewrite<-forall_interp_spred2_same_pkg.
        rewrite forall_preprocess_fancy_spreds_correct2.
        rewrite Forall_ignore_map_fst.
        constructor; [ | constructor].
        + basic_cbn. intros. vm_compute in H1. discriminate.
        + basic_cbn. intros. vm_compute in H1. discriminate.
        + rewrite Forall_concat. rewrite Forall_map. constructor.
          { eapply cache_pre_conds_implied; eauto; simpl; eauto. }
          constructor; [ | constructor].
          { eapply cache_pre_conds_implied; eauto; simpl; eauto. }
    }
    ...
    - simpl. apply wf_initial_cache_pair.
    Unshelve.
    + exact None.
    + exact dummy_input_t.
  Qed.
