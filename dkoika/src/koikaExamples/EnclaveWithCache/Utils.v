...
Open Scope list_scope.
  ...
  Definition memory_to_sm_fifos (core: ind_core_id) : list SecurityMonitor.reg_t :=
      map Memory ((map (Memory.cacheToCore imem core) FiniteType.finite_elements) ++
                  (map (Memory.cacheToCore dmem core) FiniteType.finite_elements)
                 ).

  Definition sm_to_memory_fifos (core: ind_core_id) : list SecurityMonitor.reg_t :=
      map Memory ((map (Memory.coreToCache imem core) FiniteType.finite_elements) ++
                  (map (Memory.coreToCache dmem core) FiniteType.finite_elements)
                 ).
    ...
    Definition mem_cache_regs (cache: mem_type) (core: ind_core_id) : list SecurityMonitor.reg_t :=
      map Memory (map (Memory.cache_reg cache core) (FiniteType.finite_elements)).

    Definition mem_regs (core: ind_core_id) :=
      (map Memory ([Memory.purge core; Memory.turn; Memory.priv_turn core; Memory.freeze core])) ++
      (mem_cache_regs imem core) ++
      (mem_cache_regs dmem core) ++
      (memory_to_sm_fifos core) ++
      (sm_to_memory_fifos core).

    Definition mem_regs_to_reset (core: ind_core_id) :=
      (map Memory ([Memory.priv_turn core; Memory.freeze core])) ++
      (mem_cache_regs imem core) ++
      (mem_cache_regs dmem core) ++
      (sm_to_memory_fifos core).
  ...
  Definition core_regs_to_reset core : list SecurityMonitor.reg_t :=
    remove_regs (core_regs core) ((map (core_reg core)) [core_id; init_pc; purge] ++ (core_to_sm_fifos core) ++
